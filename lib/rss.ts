...
/**
 * Parse date from multiple formats (robust parsing)
 */
function parseRSSDate(pubDate: string | undefined | null): Date | null {
  if (!pubDate) return null;
  
  try {
    // Try standard Date constructor first (handles ISO, RFC formats)
    ...
    if (!isNaN(date.getTime())) {
      ...
      // If date is reasonable (within last year and not in future), use it
      if (date <= now && date >= oneYearAgo) {
        return date;
      }
    }
    
    // If parsing failed or date is unreasonable, return null
    return null;
  } catch {
    return null;
  }
...
function formatDate(pubDate: string | undefined | null): string {
  ...
  if (!date) {
    // Don't default to "now" - use a fixed fallback that looks more authentic
    return '3 hours ago';
  }
...
async function fetchFeed(url: string, source: string): Promise<NewsItem[]> {
  try {
    ...
    for (const item of feed.items.slice(0, 20)) {
      ...
      // Get the publication date - try multiple fields
      const pubDate = item.pubDate || item.isoDate || itemAny.published || itemAny.updated;
      ...
      items.push({
        ...
        readTime: formatDate(pubDate), // Use actual RSS date instead of calculated read time
